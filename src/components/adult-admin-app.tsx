...
function StudentDetailDialog({ student, onClose, onPay, onSession, onEdit, onEntry }: { student: StudentReadModel; onClose: () => void; onPay: () => void; onSession: () => void; onEdit: () => void; onEntry: (entry: StudentHistoryEntryReadModel) => void }) {
  return <DialogShell title={student.name} description="Balance, attendance, and payment history in one place." onClose={onClose}>
    ...
    <div className="student-detail-actions">{!student.archived && <><button className="primary" onClick={onPay}>+ Payment</button><button className="secondary" onClick={onSession}>+ Session</button></>}<a className="secondary statement-link" href={`/statements/${student.id}`} target="_blank" rel="noreferrer">Generate statement</a><button className="text-button" onClick={onEdit}>{student.archived ? "Restore student" : "Edit profile"}</button></div>
