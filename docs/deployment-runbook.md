...
**Status: NOT READY / DO NOT DEPLOY.** As of 2026-08-04:

- The UI uses protected routes, authenticated live reads, and audited Supabase mutations. It contains no in-memory production data adapter or automatic demo data (SEC-001 closed).
- Audited RPC coverage is applied for student, individual session, correction/void, template, and daily-review mutations. Browser-level integration testing remains open (SEC-002).
- The two timestamped migrations are applied to project `mevsairosejypqqtfnum`; RLS/grants/functions were verified with advisors and rolled-back authorization tests (SEC-003 closed for the approved target).
- Sign-in and protected-route behavior are implemented. The coach account must still be created and public enrollment disabled/verified before deployment (SEC-004).
- Dependency links are healthy; lint, typecheck, 19 tests, and the production build pass locally.
...
Use Node.js 24 and pnpm 11.9.0 (declared in `package.json`). Dependencies are currently installed; if they need repair in a network-enabled environment:
...
1. Confirm the two timestamped migrations in `supabase/migrations/` match the Supabase migration history. Direct writes are revoked and mutations fail closed through audited RPCs.
...
4. Verify the existing timestamped migration files and their checksums against the approved review record; create a new forward-only migration for any later schema change.
5. For later changes, apply locally first. Verify schema objects, constraints, grants, indexes, function ownership/search paths, and the migration list. Never test authorization as `postgres` or service role alone.
...
- [x] Frontend uses authenticated persisted server actions; no mock adapter exists in production paths.
